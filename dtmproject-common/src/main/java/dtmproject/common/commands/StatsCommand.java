...
public class StatsCommand implements CommandExecutor {
	...
	if (args.length == 0) {
	    ...
	    sender.sendMessage("§bElo-pisteet: §4[" + pd.getRatingLevel() + "§4] §b" + (int) pd.getEloRating());
...
//			sender.sendMessage(pd.getTotalStats().toString());
	    ...
	    // Print targets' stats async
	    for (String arg : args) {
		if (arg.length() <= 16 && arg.length() >= 3) {
		    sender.sendMessage("§3>§b> §8+ §7Annoit huonon nimen: " + arg);
		    return true;
		}

		CompletableFuture<DTMPlayerData> task = pl.getDataHandler().getOfflineData(arg);
		task.whenComplete((data, error) -> {
		    if (data == null) {
			sender.sendMessage("§3>§b> §8+ §7Ei löydetty statseja pelaajalle " + args[0] + ".");
			return;
		    }
		    sender.sendMessage("§3>§b> §8+ §7§l     Tilastot - " + data.getLastSeenName());
		    sender.sendMessage(data.getSeasonStats().toString());
		    sender.sendMessage(
			    "§bElo-pisteet: §4[" + data.getRatingLevel() + "§4] §b" + (int) data.getEloRating());
		});
	    }
	}
	return true;
